...
// float3 hom_reduce_0(float4 v){return (float3(v)/v[3]);}

float4 homify_2(float3 v){return float4(v, 0.);}

float4 homify_1(float3 v){return float4(v, 0.);}

float4 homify_0(float3 v){return float4(v, 1.);}

float4x4 _gator_times_20(float4x4 m2, float4x4 m1){return (m2*m1);}

float4x4 _gator_plus_14(float4x4 m1, float4x4 m2){return (m1+m2);}

float4 _gator_times_19(float4x4 m, float4 p){return (m*p);}

float4 _gator_times_18(float4x4 m, float4 v){return (m*v);}

float4 _gator_times_17(float4x4 m, float4 v){return (m*v);}
...
float3 _gator_times_16(float s, float3 v){return (s*v);}

float3 _gator_times_15(float3 v, float s){return (v*s);}

float3 _gator_minus_17(float3 x, float3 y){return (x-y);}

float3 _gator_plus_9(float3 x, float3 y){return (x+y);}

float2 mix_2(float2 v1, float2 v2, float2 v3){return mix(v1, v2, v3);}

float2 reflect_3(float2 v1, float2 v2){return reflect(v1, v2);}

float2 reflect_2(float2 v1, float2 v2){return reflect(v1, v2);}

float2 normalNormalize_0(float2 v){return normalize(v);}
...
float2 _gator_times_14(float s, float2 v){return (s*v);}

float2 _gator_times_13(float2 v, float s){return (v*s);}

float2 _gator_minus_10(float2 x, float2 y){return (x-y);}

float2 _gator_plus_6(float2 x, float2 y){return (x+y);}

float Grayscale_float_0(float3 inp){float r = inp[0];float g = inp[1];float b = inp[2];float retval = (((r+g)+b)*0.33);return retval;}

void Grayscale_float(float inp, out float output) { output = Grayscale_float_0(inp);} 
