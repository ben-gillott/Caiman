...
void main(float3 vPosition, float4x4 uModel, float4x4 uView, float3 vNormal, float3 uLight, float uRef, float4 output) {
    ...
    // Example phong model with errors
        ...
        float3x3 modelMatrix = float3x3(uModel);
        //float3 worldPos = modelMatrix * vPosition;
        //float3 worldNorm = modelMatrix * vNormal;

        //float3 worldPos = float3(uModel * float4(vPosition, 1.));
        //float3 worldNorm = float3(uModel * float4(vNormal, 1.));
        // Uncomment for failing example
        float3 worldPos = vPosition;
        float3 worldNorm = vNormal;
        ...
        float specular = pow(max(dot(normalize(-camPos), reflectDir), 0.), 32.);
        gl_FragColor = float4(ambient + diffuse * diffColor + specular * specColor, 1.0);
        // gl_FragColor = float4(ambient + diffuse * diffColor, 1.0);
    }
}
